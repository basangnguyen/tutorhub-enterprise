...
| **Inline Completion (Ghost text)** | Gợi ý mờ ngay trong ô nhập | ✅ Đã hoàn thành | 🟡 Trung bình |
| **Highlight matched text** | Chữ khớp được **bôi đậm** | ✅ Đã hoàn thành | 🔴 Cao |
...
| **Keyboard shortcut Ctrl+K** | Focus vào ô tìm kiếm | ✅ Đã wire lên UI | 🟢 Thấp |
| **Real icons (SVG)** | Icon phù hợp cho mỗi loại | ✅ Đã hoàn thành | 🟡 Trung bình |
...
### ✅ Phase E — Inline Completion (Ghost Text) ⏱️ Đã hoàn thành

**Mục tiêu:** Khi gõ "Mở L", ô tìm kiếm hiện "Mở L<span style="color:gray">ịch</span>" (phần xám là gợi ý).

| Task | Chi tiết | Trạng thái |
|---|---|---|
| E1 | Tạo custom `GhostTextTextField` extends `JTextField` – override `paintComponent` để vẽ ghost text mờ phía sau cursor | ✅ Xong |
| E2 | Ghost text lấy từ kết quả top match của `SearchController` | ✅ Xong |
| E3 | Nhấn `Tab` hoặc `→` để accept ghost text vào ô nhập | ✅ Xong |
| E4 | Ghost text biến mất khi kết quả không khớp | ✅ Xong |

**Files mới:** `GhostTextTextField.java` (hoặc tích hợp vào `GlobalSearchBar`)

---

### ✅ Phase F — Real Icons & UI Polish ⏱️ Đã hoàn thành

**Mục tiêu:** Thay text badge (HOME, MSG, CLS...) bằng SVG icon thật cho chuyên nghiệp.

| Task | Chi tiết | Trạng thái |
|---|---|---|
| F1 | Tạo bộ icon SVG cho mỗi `SearchResultType`: 🏠 Home, 💬 Chat, 📚 Class, 📅 Calendar, 📝 Quiz, 📁 Docs, 👤 Profile, ⭐ Upgrade, 🌐 Web, 🕐 History | ✅ Xong |
| F2 | Cập nhật `ResultRow` trong `SearchDropdownWindow` render icon SVG thay text badge | ✅ Xong |
| F3 | Thêm avatar người dùng cho kết quả Chat search | ⚠️ Đã dùng SVG thay thế avatar |
| F4 | Dark mode support cho dropdown (nếu app có dark mode) | ✅ Xong (FlatLaf ColorFilter support) |
...
### ✅ Phase G — Server Search Cleanup & Advanced ⏱️ Đã hoàn thành

**Mục tiêu:** Tối ưu backend, thêm tính năng nâng cao.

| Task | Chi tiết | Trạng thái |
|---|---|---|
| G1 | Chuẩn hóa protocol `GLOBAL_SEARCH` thay vì reuse `SEARCH_USER` | ✅ Xong |
| G2 | Server-side rate limiting cho search requests (bảo vệ DB) | ✅ Xong |
| G3 | Client-side: Min query length (≥ 2 ký tự) trước khi gọi server | ✅ Xong |
| G4 | `SearchRanking.java` – Hệ thống ranking nâng cao: exact match > prefix > contains > fuzzy | ✅ Xong |
| G5 | Recently opened items (không chỉ recently searched) | ⚠️ Dùng Recently Searched thay thế |
| G6 | Wire `Ctrl+K` global shortcut để user luôn có thể focus vào search bar | ✅ Xong |
| G7 | Keyboard shortcut hints trong dropdown (hiện "↵ để mở", "Esc để đóng") | ✅ Xong |
...
### Lập kế hoạch chi tiết cho Phase D (Async Providers & Loading Skeleton)

> [!NOTE]
> Phase D sẽ chuyển đổi kiến trúc tìm kiếm từ **Synchronous** sang **Asynchronous**, giúp thanh tìm kiếm không bao giờ bị "đơ" UI khi phải quét một lượng dữ liệu lớn hoặc gọi API.

**Các công việc cần làm:**

#### 1. Đổi Interface `SearchProvider` sang Async
- Thay đổi chữ ký phương thức: `List<SearchResult> search(...)` $\rightarrow$ `CompletableFuture<List<SearchResult>> searchAsync(...)`.

#### 2. Cập nhật tất cả Providers hiện tại
- Các Provider đang chạy nhanh (History, Command, Web, Class, Blackboard, Chat) sẽ được cập nhật để trả về `CompletableFuture.supplyAsync(() -> { ... })`.
- Các xử lý string matching (so sánh text, fuzzy search) giờ đây sẽ chạy trên thread pool (ForkJoinPool mặc định), giảm tải hoàn toàn cho EDT (UI Thread).

#### 3. Cập nhật `SearchController`
- Hàm gom kết quả `executeSearch` $\rightarrow$ `executeSearchAsync`.
- Dùng `CompletableFuture.allOf(...)` để gom kết quả từ tất cả các Providers đang thỏa điều kiện `supports(query)`.
- **An toàn & Timeout:** Áp dụng `completeOnTimeout(emptyList(), 3, TimeUnit.SECONDS)` cho mỗi provider. Nếu provider nào chạy lâu hơn 3 giây (ví dụ sau này gọi API rớt mạng), nó sẽ tự động bị bỏ qua, không làm treo toàn bộ dropdown.

#### 4. Cập nhật `GlobalSearchBar` (Quản lý Cancelling)
- Quản lý request đang chạy (`currentSearchFuture`).
- Nếu user gõ thêm ký tự mới khi request cũ chưa xong $\rightarrow$ gọi `currentSearchFuture.cancel(true)` để hủy bỏ việc update giao diện từ request lỗi thời.
- Gọi UI cập nhật thông qua `SwingUtilities.invokeLater` khi future thành công.

#### 5. Cập nhật `SearchDropdownWindow` (Loading Skeleton)
- Thêm phương thức `showLoadingState(x, y, query)`: Nếu data chưa về ngay lập tức, hiển thị giao diện chờ (Loading Skeleton).
- Giao diện chờ: Sẽ vẽ các JPanel màu xám nhạt (placeholder) mô phỏng các kết quả đang load, tạo cảm giác mượt mà (perceived performance) thay vì để một màn hình trắng.

> [!WARNING]
> Do thay đổi interface gốc `SearchProvider`, mọi provider đều sẽ bị lỗi biên dịch cho tới khi tôi cập nhật xong. Tôi sẽ commit và push đầy đủ ngay khi sửa xong lỗi biên dịch của phần này.

Bạn có đồng ý với kế hoạch triển khai Phase D này không? Nếu có bất kỳ điều chỉnh nào về UX của trạng thái Loading (ví dụ: muốn dùng hiệu ứng Shimmer hay Spinner), hãy cho tôi biết!
