(function () {
    var msg = "[TSE_QS_PARENT_JS] LOADED version=FULL_BRIGHTNESS_RESEARCH_FIX";
    try {
        console.log(msg);
...
window.updateState = function(jsonStr) {
    ...
    try {
        ...
        if (wifi) {
            if (data.wifiSsid) {
                wifi.innerText = data.wifiSsid + " (Đang kết nối)";
            } else {
                wifi.innerText = data.wifiText || "Không kết nối";
            }
        }
...
function onVolumeInput(val) {
    logToJava("[TSE_QS_PARENT_JS] onVolumeInput called with val=" + val);
    ...
    sliderState.volume.debounceTimer = setTimeout(function() {
        logToJava("[TSE_QS_PARENT_JS] onVolumeInput debounce fired, sending setVolumeCommand");
        if (window.javaApp && window.javaApp.setVolumeCommand) {
            ...
            logToJava("[TSE_QS_PARENT_JS] send setVolumeCommand payload=" + payload);
            window.javaApp.setVolumeCommand(payload);
        } else {
            logToJava("[TSE_QS_PARENT_JS] window.javaApp.setVolumeCommand NOT FOUND in onVolumeInput");
        }
    }, 150);
}
...
function onVolumeChange(val) {
    ...
    if (window.javaApp && window.javaApp.setVolumeCommand) {
        ...
        logToJava("[TSE_QS_PARENT_JS] window.javaApp.setVolumeCommand NOT FOUND");
    }
    syncVolumeUI();
...
function toggleMute() {
    ...
    if (window.javaApp && window.javaApp.setMutedCommand) {
        ...
        logToJava("[TSE_QS_PARENT_JS] send setMutedCommand payload=" + payload);
        window.javaApp.setMutedCommand(payload);
    } else {
        logToJava("[TSE_QS_PARENT_JS] window.javaApp.setMutedCommand NOT FOUND");
    }
    syncVolumeUI();
    sliderState.volume.mode = "COMMITTED";
}
...
function onAppReady() {
    logToJava("[TSE_QS_PARENT_JS] LOADED version=FULL_BRIGHTNESS_RESEARCH_FIX");
...
function showStatusMessage(type) {
    var statusEl = document.getElementById("tse-" + type + "-status");
    if (!statusEl) return;
    statusEl.innerText = "Đang chỉnh...";
    statusEl.style.display = "block";
    statusEl.style.color = "#aaa";
    if (statusEl.timeout1) clearTimeout(statusEl.timeout1);
    if (statusEl.timeout2) clearTimeout(statusEl.timeout2);
    statusEl.timeout1 = setTimeout(function() {
        statusEl.innerText = "Đã lưu";
        statusEl.style.color = "#60CDFF";
        statusEl.timeout2 = setTimeout(function() {
...
function onTestSoundClick() {
    if (window.javaApp && window.javaApp.testSoundCommand) {
        logToJava("[TSE_QS_PARENT_JS] calling javaApp.testSoundCommand()");
        window.javaApp.testSoundCommand();
    } else {
        logToJava("[TSE_QS_PARENT_JS] window.javaApp.testSoundCommand NOT FOUND");
    }
}
