...
(function () {
  ...
  function setupVisualEffects() {
    ...
    // 2–3 bursts around the logo shortly after load.
    [180, 560, 940].forEach(function (delay, idx) {
      setTimeout(function () {
        var c = heroCenter();
        var offsetX = idx === 1 ? -90 : (idx === 2 ? 90 : 0);
        launchFirework(c.x + offsetX, c.y + rand(-20, 10), 50);
      }, delay);
    });
    ...
    function applyParallax() {
      ...
      for (var c = 0; c < cards.length; c++) {
        ...
        cards[c].style.transform =
          'translate(' + (pendingX * depth).toFixed(1) + 'px,' +
          (pendingY * depth).toFixed(1) + 'px)';
      }
    }
